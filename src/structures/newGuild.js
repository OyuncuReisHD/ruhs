const newGuild = (async (guildData) => {
  ...
  const newPresence = require("./newPresence.js");
  ...
  guildData.members.forEach((memberData) => {
    if (guildData.members.filter((value) => value.id === memberData.id).size !== 1) {
      members.set(memberData.user.id, newMember(memberData));
    }
  });
  ...
  if (members.has(guildData.owner_id)) {
    ...
    members.set(guildData.owner_id, newMember(ownerMember));
